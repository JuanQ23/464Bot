...
async def create_channels(interaction: discord.Interaction, member: discord.Member):
    ...
    # The Person who ran the command can see the category and the member passed in can see the
    # category.
    overwrites = {
...
async def delete_groups(category: discord.CategoryChannel):
    """A helper that assists in the deletetion of categories along with their channels.
    kind of like recursively delete a folder on windows."""
...
class ChannelSelectView(discord.ui.View):
    ...
    def __init__(self):
        ...
        # a attribute that will hold the user selected channels. ignore the LONG type hint.
        # its only there to keep the type checker happy.
        self.value: List[app_commands.AppCommandChannel | app_commands.AppCommandThread]

    @discord.ui.select(cls=discord.ui.ChannelSelect, placeholder="search", min_values=1,
                       max_values=20)
    async def my_user_channels(self, interaction: discord.Interaction,
                               select: discord.ui.ChannelSelect):
        """A callback that is invoked whenever a user selects something in the channel select Menu.

        This function is made into a callback via the @discord.ui.select decorator. It will return
        The value to wherever the instance of the class ChannelSelectView is created.

        Args:
            interaction (discord.Interaction): Event parameter...
            select (discord.ui.ChannelSelect): Event parameter...
        """
...
class PrivateRooms(commands.Cog):
    """A set of commands that are used to automate creation and deletion of private text
    and voice channels between a student and their professor"""
    ...
    async def create(self, interaction: discord.Interaction):
        """Creates private categories, private text channels, private voice channels for each
        student"""
        ...
        # Create categories and channels for each member. If the category is already there, then
        # skip its creation
        ...
        for member in members:
            if member.nick in category_names:
                continue
            if member.name in category_names:
                continue
    ...
    async def destroy(self, interaction: discord.Interaction):
        ...
        # collecting response. which is a list of channels selected by user.
        to_delete = view.value

        # ugly one liner that converts partial channel objects into their full formed respective
        # object. categories is a list of categories selected by the user via the select view sent
        # to discord.
        categories = [await element.fetch() for element in to_delete
                      if element.type == discord.ChannelType.category]
    ...
    async def destroy_all_except(self, interaction: discord.Interaction):
        ...
        # ugly one liner that converts partial channel objects into their full formed respective
        # object.
        # categories is a list of categories selected by the user via the select view sent to
        # discord.
        categories = [await element.fetch() for element in to_save
                      if element.type == discord.ChannelType.category]
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        *guild avatar
        ...
        This functions monitors for changes in Nickname and updates the category to nickname.
        If there is a change in nickname then it changes the students category name to.
    ...
    async def on_ready(self):
        """A event that is called when the cog is ready.

        This function is turned into a event via the @commands.Cog.listener decorator.
        """
        print("PrivateRooms cog loaded")


async def setup(bot):
    """This function adds the cog to the bot so that it it's command can be ran and the events can
    be listned for."""
